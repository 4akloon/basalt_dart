...
- 🚚 **CLI** — `basalt migration generate/run/revert/redo`, `database reset`, `print-schema`; migrations are tracked in `__basalt_schema_migrations`.
...
import 'schema.dart'; // generated by `basalt print-schema`
...
| [`basalt_cli`](packages/basalt_cli) | The **`basalt`** executable: migrations + `print-schema`. |
