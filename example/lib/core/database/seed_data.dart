...
/// single transaction. Each table is one batch `INSERT` built by the generated
/// `Iterable.toInsert()` derive (the write models), so the primary keys
/// **autoincrement** — the ids referenced by the foreign keys below (`1..N`)
/// line up because a multi-row `VALUES` inserts rows in list order per table.
class SeedData {
  const SeedData._();
  ...
  /// Inserts the demo data in one transaction — one batch insert per table.
    ...
    await db.transaction((tx) async {
      ...
      await tx.execute([
        for (final (name, parent) in const [
          ...
          CategoryWrite(name: name, parentId: parent),
      ].toInsert());
      ...
      await tx.execute([
        for (final (name, desc, price, stock, cat, metadata) in const <(
          String,
          String,
          double,
          int,
