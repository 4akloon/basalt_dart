...
/// Installs the `ext.basalt.*` VM service extensions the DevTools UI calls.
///
/// Called once, lazily, from [BasaltDevTools.register]. Each handler receives
/// string-valued params (the VM service transport) and returns a JSON string.
void installServiceExtensions() {
  _register(BasaltExtension.listInstances.method, (_) async {
  ...
  _register(BasaltExtension.getSchema.method, (p) async {
    return (await _service.getSchema(_require(p, 'id'))).toJson();
  });

  _register(BasaltExtension.getTableData.method, (p) async {
  ...
  _register(BasaltExtension.updateRow.method, (p) async {
  ...
  _register(BasaltExtension.runSql.method, (p) async {
