...
`SqlType<T>` defines how a Dart value is `encode`d into a driver parameter and
`decode`d back. The built-in instances are `const` (which is what lets columns
be `static const` and usable in annotations).
...
For columns that allow `NULL`, use the `*OrNull` variants — the column type
becomes `T?`:

`SqlType.integerOrNull`, `textOrNull`, `realOrNull`, `booleanOrNull`,
`blobOrNull`, `dateTimeOrNull`.
...
`SqlType<T>` **is** the codec extension point — its constructor takes
`encode`/`decode`. Keep it `const` by using top-level tear-off codecs, so it
works in `static const` columns. For example, an enum stored by name:
