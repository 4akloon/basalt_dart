...
> Because the generated class extends `TableRef`, a column whose camelCase
> field name collides with an inherited member (`table`, `tableName`, `alias`,
> `columns`, `col`, `aliased`, or an `Object` member like `to_string`) cannot
> be emitted — `generate-schema` fails with a clear error asking you to rename
> the column.
