...
`generate-schema` introspects the live database, so run it **after** applying migrations. It writes table marker
classes to `schema_output` in `basalt.yaml` (default `lib/schema.dart`):
